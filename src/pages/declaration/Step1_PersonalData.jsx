...
function Step1_PersonalData(/* { initialData, onNext, onSaveAndExit } */) {
  const navigate = useNavigate();

  // --- Estados locales para los campos de este paso ---
  // Inicializar con datos del usuario si vinieran de API/Context, sino vacíos
  ...
  useEffect(() => {
    console.log('Cargando datos iniciales para Paso 1...');
  }, []); 
  ...
  const handleNextStep = () => {
    console.log('Datos Paso 1:', formData);

    alert('Datos guardados (simulado). Pasando al siguiente paso...');
    navigate('/declaration/income'); 
  };

  // Lógica Guardar y Salir
  const handleSaveAndExit = () => {
    console.log('Guardando datos Paso 1 y saliendo:', formData);
    alert('Progreso guardado (simulado). Volviendo al dashboard...');
    navigate('/dashboard');
  };
