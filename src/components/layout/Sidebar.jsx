...
function Sidebar({ isOpen, toggleSidebar }) {
  ...
  return (
    <aside className={`bg-white border-r border-gray-200 h-screen overflow-y-auto sticky top-0 transition-all duration-300 ease-in-out ${isOpen ? 'w-64' : 'w-20'}`}>
       {/* Quitamos padding 'p-4' del contenedor principal para controlar header/nav por separado */}
      <div>
        {/* --- Header de la Sidebar --- */}
        <div className={`flex items-center p-4 ${isOpen ? 'justify-between' : 'justify-center'}`}> {/* Ajusta justificado */}
          {/* Logo (Visible solo si está abierto) */}
          <span className={`font-bold text-xl text-indigo-600 ${isOpen ? 'inline-block' : 'hidden'}`}>
            ContApp
          </span>
          {/* Botón Hamburguesa (Siempre visible) */}
          <button
            onClick={toggleSidebar} // Llama a la función del padre
        ...
        <nav className="mt-2 px-2 space-y-1"> {/* Ajustado padding/margen */}
